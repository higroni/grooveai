...
### 2. Ontology je Rečnik Domena

**Stari pristup:**
- Ontology 1:1 sa Corpus

**Novi pristup:**
- **Ontology 1:N sa Domain** (jedan domen može imati više ontologija/rečnika)
- **Ontologija je samo rečnik termina** - ne vezuje se za korpus

**Razlog:**
- Ontologija je domen-specifični rečnik termina i sinonima
- Svi korpusi u domenu automatski koriste ontologiju tog domena
- Jednostavniji model - manje veza

**Primer:**
```
Domain: "Radno pravo"
  ├─ Ontology: "Radno pravo - Osnovna"
  │   └─ Terms: {zaposleni: [radnik, službenik], ...}
  ├─ Ontology: "Radno pravo - Proširena"
  │   └─ Terms: {zaposleni: [radnik, službenik, lice u radnom odnosu], ...}
  └─ Corpora:
      ├─ "Pilot Radni Odnosi" (koristi ontologiju d omena)
      ├─ "Zakon o radu 2024" (koristi ontologiju domena)
      └─ "Pravilnici o radu" (koristi ontologiju domena)
```
...
**Opis:** Pravna oblast koja grupiše srodne propise i regulativu.
...
**Odnosi:**
- **1:N sa Ontology** - Ima više ontologija/rečnika
...
```json
{
  "id": "domain-radno-pravo",
  "name": "Radno pravo",
  "description": "Propisi koji regulišu radne odnose, prava i obaveze poslodavaca i zaposlenih, bezbednost na radu, i rešavanje radnih sporova",
  "default_conflict_set_id": "set-standard-127",
...
### 2. Ontology (Ontologija/Rečnik)

**Opis:** Rečnik pravnih termina i njihovih sinonima specifičan za pravnu oblast. Koristi se za poboljšanje ekstrakcije asercija i semantičkog pretraživanja.

**Atributi:**
- `id` (UUID)
- `domain_id` (UUID) - Pripada domenu
- `name` (string) - "Radno pravo - Osnovna"
- `terms` (JSON) - Termini i sinonimi
- `concepts` (JSON) - Hijerarhija koncepata (opciono)
- `rules` (JSON) - Pravila za ekstrakciju (opciono)
- `version` (int)
- `is_active` (boolean) - Da li se trenutno koristi
- `created_at` (datetime)

**Odnosi:**
- **N:1 sa Domain** - Pripada jednom domenu
- **Implicitno koristi se za sve korpuse u tom domenu**

**Pravni primer:**
```json
{
  "id": "onto-radno-pravo",
  "name": "Radno pravo - Osnovna ontologija",
  "terms": {
    "zaposleni": ["radnik", "službenik", "lice u radnom odnosu"],
    "poslodavac": ["pravno lice", "preduzeće", "poslodavac", "organ"],
    "ugovor o radu": ["radni odnos", "ugovor o zaposlenju"],
    "otkaz": ["prestanak radnog odnosa", "raskid ugovora"]
  },
  "concepts": {
    "radni_odnos": {
      "parent": null,
      "children": ["zasnivanje", "trajanje", "prestanak"],
      "related": ["ugovor_o_radu", "prava_zaposlenih", "obaveze_poslodavca"]
    }
  },
  "rules": {
    "obligation_patterns": ["dužan je", "mora", "obavezan je"],
    "prohibition_patterns": ["ne sme", "zabranjeno je", "nije dozvoljeno"],
    "deadline_patterns": ["u roku od", "najkasnije do", "do"]
  },
  "version": 2
}
```
...
**Opis:** Kolekcija pravnih propisa iz određene oblasti koja služi kao referentna baza za analizu usklađenosti.
...
**Odnosi:**
- **N:1 sa Domain** - Pripada domenu (i automatski koristi njegovu ontologiju)
- **1:N sa Document**

**Pravni primer:**
```json
{
  "id": "corpus-radno-pravo-2024",
  "name": "Radno pravo - Aktuelni propisi 2024",
  "domain_id": "domain-radno-pravo",
  "description": "Kompletna kolekcija važećih propisa iz oblasti radnog prava: Zakon o radu, pravilnici, uredbe i kolektivni ugovori",
  "status": "ready",
  "documents_count": 23,
  "assertions_count": 1847,
...
**Opis:** Numerička vektorska reprezentacija pravne asercije koja omogućava semantičko poređenje i pretragu.
...
{
  "id": "emb-assert-12345",
  "assertion_id": "assert-zakon-o-radu-108",
  "vector": [0.023, -0.156, 0.089, ...],  // 1024 dimenzije
  "model": "BAAI/bge-m3",
  "dimensions": 1024,
  "created_at": "2024-01-15T11:30:00Z"
}
// Ovaj vektor predstavlja semantiku asercije:
// "Poslodavac je dužan da zaposlenom isplati zaradu najkasnije do 15. u mesecu"
// Omogućava pronalaženje sličnih obaveza u drugim propisima
```
...
**Opis:** Definicija logike za detekciju specifičnog tipa pravnog konflikta između propisa.
...
```json
{
  "id": "rule-contradictory-deadline",
  "conflict_set_id": "set-standard-127",
  "type": "contradictory_deadline",
  "category": "temporal",
  "name": "Konfliktni rokovi za istu obavezu",
  "description": "Detektuje kada dva propisa propisuju različite rokove za izvršenje iste pravne obaveze",
  "detection_logic": {
    "require_match": ["subject", "action"],
    "require_difference": ["deadline"],
    "similarity_threshold": 0.80,
    "example": "Zakon propisuje rok 15 dana, pravilnik 30 dana"
  },
  "severity": "high",
  "enabled": true,
  "version": 1
