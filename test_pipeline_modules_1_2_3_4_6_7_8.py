"""
Integration test for Modules 1, 2, 3, 4, 6, 7, and 8.
Tests the complete pipeline: File Reader -> Text Normalizer -> Latinizer -> Legal Parser -> Assertion Extractor -> Entity Recognizer -> Condition Extractor
"""
...
def test_full_pipeline():
    """Test the complete pipeline from PDF to extracted conditions."""
    
    print("=" * 80)
    print("FULL PIPELINE TEST: Modules 1 -> 2 -> 3 -> 4 -> 6 -> 7 -> 8")
    ...
    # Step 8: Extract conditions from assertions using Module 8
    print(f"\n[STEP 8] Extracting conditions from assertions using Module 8...")
    print(f"[INFO] Processing {len(all_assertions)} assertions")
    ...
    print("\n[PREVIEW] Sample entities (first 10):")
    for i, entity in enumerate(all_entities[:10], 1):
        print(f"\n  {i}. Type: {entity['entity_type']}")
    ...
    print("\n[PREVIEW] Sample conditions (first 10):")
    for i, condition in enumerate(all_conditions[:10], 1):
        print(f"\n  {i}. Type: {condition['condition_type']}")
        print(f"     Trigger: '{condition['trigger_word']}'")
        print(f"     Text: '{condition['text'][:80]}...'")
        print(f"     Confidence: {condition['confidence']:.2f}")
    
    # Save complete pipeline output
    output_file = "pipeline_output_with_conditions.json"
    print(f"\n[SAVING] Writing complete pipeline output to {output_file}...")
    
    pipeline_output = {
        "pipeline": "Modules 1 -> 2 -> 3 -> 4 -> 6 -> 7 -> 8",
    ...
    # Save entities and conditions as readable text
    output_file_txt = "pipeline_output_conditions.txt"
    print(f"[SAVING] Writing entities and conditions to {output_file_txt}...")
    
    with open(output_file_txt, "w", encoding="utf-8") as f:
        f.write("=" * 80 + "\n")
        f.write("EXTRACTED ENTITIES AND CONDITIONS FROM LEGAL DOCUMENT\n")
        f.write("=" * 80 + "\n\n")
        ...
        f.write(f"Total Conditions Extracted: {len(all_conditions)}\n\n")
        f.write("=" * 80 + "\n\n")
        ...
        for assertion in all_assertions:
            assertion_id = assertion['assertion_id']
            assertion_entities = [e for e in all_entities if any(
                job['assertion_id'] == assertion_id 
                for job in recognition_jobs
            )]
            
            if len(assertion_entities) > 0:
                f.write(f"ASSERTION: {assertion['text']}\n")
                f.write(f"Confidence: {assertion['confidence']:.2f}\n")
                f.write(f"Entities: {len(assertion_entities)}\n")
                f.write("-" * 80 + "\n\n")
                
                for i, entity in enumerate(assertion_entities, 1):
                    f.write(f"  {i}. {entity['entity_type']}: '{entity['text']}'\n")
                    f.write(f"     Confidence: {entity['confidence']:.2f}\n")
                    f.write(f"     Position: {entity['start_char']}-{entity['end_char']}\n\n")
        ...
        for assertion in all_assertions[:5]:  # Only first 5 assertions
            assertion_id = assertion['assertion_id']
            assertion_conditions = [c for c in all_conditions if any(
                job['assertion_id'] == assertion_id 
                for job in condition_jobs
            )]
            
            if len(assertion_conditions) > 0:
                f.write(f"ASSERTION: {assertion['text']}\n")
                f.write(f"Confidence: {assertion['confidence']:.2f}\n")
                f.write(f"Conditions: {len(assertion_conditions)}\n")
                f.write("-" * 80 + "\n\n")
                
                for i, condition in enumerate(assertion_conditions, 1):
                    f.write(f"  {i}. {condition['condition_type'].upper()}: '{condition['trigger_word']}'\n")
                    f.write(f"     Text: {condition['text']}\n")
                    f.write(f"     Confidence: {condition['confidence']:.2f}\n")
                    f.write(f"     Position: {condition['start_char']}-{condition['end_char']}\n\n")
    ...
    print(f"  - {output_file_txt} (entities and conditions text)")
