"""
Integration test for Modules 1, 2, 3, 4, 6, 7, 8, 9, and 10.
...
KNOWLEDGE_ENRICHMENT_URL = "http://localhost:8110"
...
def test_full_pipeline():
    ...
    module_timings['M1_File_Reader'] = time.time() - start_time
    
    print(f"[OK] Job ID: {job1_id}")
    print(f"[OK] Extracted {len(raw_text)} characters")
    print(f"[OK] Module time: {module_timings['M1_File_Reader']:.3f}s")
    ...
    module_timings['M2_Text_Normalizer'] = time.time() - start_time
    
    print(f"[OK] Job ID: {job2_id}")
    print(f"[OK] Normalized {len(normalized_text)} characters")
    print(f"[OK] Module time: {module_timings['M2_Text_Normalizer']:.3f}s")
    ...
    module_timings['M3_Latinizer'] = time.time() - start_time
    
    print(f"[OK] Job ID: {job3_id}")
    print(f"[OK] Latinized {len(latinized_text)} characters")
    print(f"[OK] Module time: {module_timings['M3_Latinizer']:.3f}s")
    ...
    module_timings['M6_Assertion_Extractor'] = time.time() - start_time
    
    print(f"\n[OK] Extraction complete!")
    print(f"[OK] Total assertions extracted: {len(all_assertions)}")
    print(f"[OK] Total extraction jobs: {len(extraction_jobs)}")
    print(f"[OK] Module time: {module_timings['M6_Assertion_Extractor']:.3f}s")
    ...
    module_timings['M7_Entity_Recognizer'] = time.time() - start_time
    ...
    print(f"[OK] Module time: {module_timings['M7_Entity_Recognizer']:.3f}s")
    ...
    module_timings['M9_Assertion_Classifier'] = time.time() - start_time
    ...
    print(f"[OK] Module time: {module_timings['M9_Assertion_Classifier']:.3f}s")
    
    # Step 10: Enrich assertions with knowledge
    print(f"\n[STEP 10] Enriching assertions with knowledge...")
    print(f"[INFO] Processing {len(all_assertions)} assertions")
    ...
    for i, assertion in enumerate(all_assertions, 1):
        ...
        # Get classification for this assertion
        classification = next((c for c in all_classifications if c.get('assertion_id') == assertion_id), None)
        if not classification:
            # Find by matching text
            classification = all_classifications[i-1] if i <= len(all_classifications) else None
        
        # Get entities for this assertion
        assertion_entities = [e for e in all_entities if e.get('assertion_id') == assertion_id]
        
        print(f"  [{i}/{len(all_assertions)}] Enriching assertion {assertion_id[:8]}...")
        
        try:
            # Convert assertion_id to integer hash for M10 (it expects int, not UUID)
            assertion_id_int = hash(assertion_id) % (10 ** 8)  # Use hash to get consistent int
            
            enrichment_request = {
                "assertion_id": assertion_id_int,
                "assertion_text": assertion_text,
                "entities": assertion_entities,
                "use_classla": True,
                "metadata": {
                    "original_assertion_id": assertion_id,
                    "confidence": assertion.get('confidence', 0.5),
                    "assertion_type": classification['assertion_type'] if classification else "unknown"
                }
            }
            
            response10 = requests.post(
                f"{KNOWLEDGE_ENRICHMENT_URL}/enrich",
            ...
            if response10.status_code == 200:
                ...
                if data10.get('success') and data10.get('enriched_assertion'):
                    enrichment = data10['enriched_assertion']
                    
                    enrichment_jobs.append({
                        'assertion_id': assertion_id,
                        'matched_terms': len(enrichment['matched_terms']),
                        'resolved_references': len(enrichment.get('legal_references', [])),
                        'extracted_definitions': len(enrichment.get('term_definitions', [])),
                        'processing_time_ms': enrichment.get('processing_time_ms', 0)
                    })
                    
                    all_enrichments.append(enrichment)
                    
                    # Update statistics
                    enrichment_stats['matched_terms'] += len(enrichment['matched_terms'])
                    enrichment_stats['resolved_references'] += len(enrichment.get('legal_references', []))
                    enrichment_stats['extracted_definitions'] += len(enrichment.get('term_definitions', []))
                    
                    print(f"       -> Terms: {len(enrichment['matched_terms'])}, "
                          f"Refs: {len(enrichment.get('legal_references', []))}, "
                          f"Defs: {len(enrichment.get('term_definitions', []))}")
                else:
                    print(f"       -> ERROR: {data10.get('error', 'Unknown error')}")
            else:
                print(f"       -> ERROR: Status {response10.status_code}")
                
        except Exception as e:
            print(f"       -> ERROR: {str(e)}")
    
    module_timings['M10_Knowledge_Enrichment'] = time.time() - start_time
    
    print(f"\n[OK] Knowledge enrichment complete!")
    print(f"[OK] Total assertions enriched: {len(all_enrichments)}")
    print(f"[OK] Total enrichment jobs: {len(enrichment_jobs)}")
    print(f"[OK] Module time: {module_timings['M10_Knowledge_Enrichment']:.3f}s")
    ...
    pipeline_output = {
        "pipeline": "Modules 1 -> 2 -> 3 -> 4 -> 6 -> 7 -> 8 -> 9 -> 10",
        ...
        "jobs": {
            ...
            "knowledge_enrichment": [job['job_id'] for job in enrichment_jobs]
        },
        "statistics": {
    ...
    print(f"Module 10 (Knowledge Enrichment):")
    print(f"  - Assertions enriched:       {len(enrichment_jobs):>10}")
    print(f"  - Total enrichments:         {len(all_enrichments):>10}")
    print(f"  - Matched terms:             {enrichment_stats['matched_terms']:>10}")
    print(f"  - Resolved references:       {enrichment_stats['resolved_references']:>10}")
    print(f"  - Extracted definitions:     {enrichment_stats['extracted_definitions']:>10}")
    print(f"  - Additional entities:       {enrichment_stats['additional_entities']:>10}")
    print("=" * 80)
    
    # Display module timing summary
    print("\n" + "=" * 80)
    ...
    for module_name, duration in module_timings.items():
        ...
        print(f"{module_name:30} {duration:8.3f}s  ({percentage:5.1f}%)")
    print("-" * 80)
